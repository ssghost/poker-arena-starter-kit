...
def decide(table: dict, deadline_s: float = 10.0,
    ...
    if allowed.get("canBet"):
        ...
        if true_monster and equity >= 0.80:
            ...
            max_b = int(br.get("max") or min_b)
            size = compute_kelly_bet_size(equity, pot, min_b, max_b, fraction=kelly_f)
            return _build("bet", size, table, allowed,
                          eq=equity, po=0, msg="True Monster Kelly Bet")
        ...
        if equity > 0.72:
            ...
            size = compute_kelly_bet_size(equity, pot, min_b, max_b, fraction=kelly_f)
            return _build("bet", size, table, allowed,
                          eq=equity, po=0, msg="Solid Value Kelly Bet")
        ...
        if is_flop and in_pos and not has_3flush_board and (tptk or (equity >= 0.45 and t in ("P", "S", "M"))):
            ...
            return _build("bet", size, table, allowed,
                          eq=equity, po=0, msg="Flop Positional C-Bet")
        ...
        if is_turn and tptk and not has_3flush_board and equity >= 0.65:
            ...
            size = compute_kelly_bet_size(equity, pot, min_b, max_b, fraction=kelly_f)
            return _build("bet", size, table, allowed,
                          eq=equity, po=0, msg="Turn TPTK Dry Value Kelly Bet")

        if (not has_made_pair and not draw) or is_underpair:
            if "check" in available:
                return _build("check", None, table, allowed,
                              eq=equity, po=0, msg="Air/Underpair Check")

        if is_turn and not true_monster and not tptk:
            if "check" in available:
                return _build("check", None, table, allowed,
                              eq=equity, po=0, msg="Turn Pot Control Check")

...
def decide(table: dict, deadline_s: float = 10.0,
        ...
        if call_chips >= bb * 10:
            ...
            if cls in ("AA", "KK"):
                if allowed.get("canRaise"):
                    ...
                    max_r = int(rr.get("max") or min_r)
                    size = min(max_r, max(min_r, int(pot * 0.85) + call_chips))
            ...
            if t in ("P", "S") or (t == "M" and (in_pos or equity > pot_odds + 0.05)):
                return _build("call", None, table, allowed,
                              eq=equity, po=pot_odds, msg="Call PF Solid")
    ...
    if call_chips > 0:
        ...
        if true_monster and equity >= stack_off_req and allowed.get("canRaise"):
            ...
            min_r = int(rr.get("min") or call_chips * 2)
            max_r = int(rr.get("max") or min_r)
            size = min(max_r, max(min_r, int(pot * 1.35) + call_chips))
            return _build("raise", size, table, allowed,
    ...
    if allowed.get("canBet"):
        if is_flop and not in_pos and monster and equity >= 0.78:
            if "check" in available:
                return _build("check", None, table, allowed,
                              eq=equity, po=0, msg="OOP Flop Monster Trap Check")

        if true_monster and equity >= 0.80:
            br = allowed.get("betRange") or {}
            min_b = int(br.get("min") or pot // 2 or 1)
            max_b = int(br.get("max") or min_b)
            size = min(max_b, max(min_b, int(pot * 1.40)))
            return _build("bet", size, table, allowed,
                          eq=equity, po=0, msg="True Monster Overbet")
        ...
        if is_flop and in_pos and not has_3flush_board and (tptk or (equity >= 0.45 and t in ("P", "S", "M"))):
            br = allowed.get("betRange") or {}
            min_b = int(br.get("min") or max(pot // 3, 2))
            max_b = int(br.get("max") or min_b)
            size = min(max_b, max(min_b, int(pot * 0.33)))
            return _build("bet", size, table, allowed,
                          eq=equity, po=0, msg="Flop Positional C-Bet (Small Win Gen)")

        if is_turn and tptk and not has_3flush_board and equity >= 0.65:
            br = allowed.get("betRange") or {}
            min_b = int(br.get("min") or max(pot // 2, 2))
            max_b = int(br.get("max") or min_b)
            size = min(max_b, max(min_b, int(pot * 0.65)))
            return _build("bet", size, table, allowed,
                          eq=equity, po=0, msg="Turn TPTK Dry Value Bet")

        if (not has_made_pair and not draw) or is_underpair:
            if "check" in available:
                return _build("check", None, table, allowed,
                              eq=equity, po=0, msg="Air/Underpair 100% Check")

        if is_turn and not true_monster and not tptk:
            if "check" in available:
                return _build("check", None, table, allowed,
                              eq=equity, po=0, msg="Turn Pot Control Check (Anti-Double-Barrel)")
